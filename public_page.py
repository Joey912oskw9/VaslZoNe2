...
.chart-tabs{{display:flex;gap:5px;margin-top:8px;flex-wrap:wrap}}
.chart-tab{padding:4px 10px;border-radius:5px;font-size:9px;font-weight:700;background:rgba(255,215,0,0.08);color:var(--text2);border:1px solid var(--border);cursor:pointer;font-family:inherit}
.chart-tab.on{{background:var(--accent);color:#000;border-color:var(--accent)}}
<style>
...
<script>var _m=document.getElementById('bgm');_m.volume=0.08;</script>

<div id="rocket" onclick="fireRocket()" style="position:fixed;left:8px;top:50%;z-index:50;font-size:36px;color:var(--accent);cursor:pointer;filter:drop-shadow(0 0 10px rgba(255,215,0,0.3));animation:fl 3s ease-in-out infinite">🚀</div>
<div id="rex" style="position:fixed;width:80px;height:80px;pointer-events:none;z-index:60;opacity:0;border-radius:50%;background:radial-gradient(circle,#ffd700,transparent 70%)"></div>-
...
function fireRocket(){{
  ...
  setTimeout(function(){{
    var e=document.getElementById('rex');
    e.style.left='10px';e.style.top=(window.innerHeight/2-40)+'px';
    e.style.opacity='1';e.style.animation='rExplode .7s ease-out forwards';
    r.style.display='none';
    setTimeout(function(){{
      e.style.opacity='0';e.style.animation='';r.style.display='';r.style.animation='';r.dataset.f='';
    }},800);
  }},1300);
}}
...
function initChart(){{
  ...
  var v=[];for(var i=0;i<24;i++)v.push(Math.random()*50);
  chart=new Chart(c.getContext('2d'),{{
    type:'line',
    data:{{labels:Array(24).fill(''),datasets:[{{label:'MB',data:v,borderColor:'#ffd700',backgroundColor:'rgba(255,215,0,0.1)',fill:true,tension:.4,pointRadius:2,borderWidth:2}}]}},
    options:{{
      responsive:true,maintainAspectRatio:false,
      plugins:{{legend:{{display:false}},tooltip:{{backgroundColor:'rgba(13,13,10,0.95)',titleColor:'#ffd700',bodyColor:'#c8c8a0',padding:10,cornerRadius:8,displayColors:false,bodyFont:{{size:11}}}}}},
      scales:{{x:{{grid:{{display:false}},ticks:{{color:'#6b6b40',font:{{size:8}}}}}},y:{{grid:{{color:'rgba(255,215,0,0.05)'}},ticks:{{color:'#6b6b40',font:{{size:8}},callback:function(v){{return v+'MB'}}}}}}}}
  }});
}}
